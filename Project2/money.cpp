...
void Money::Subtraction(Money& a, Money& b) const
{
	Money x;
	if (x.get_sum1(a) > x.get_sum2(b))
	{
		cout << " Subtraction = " << setprecision(3) << x.get_sum1(a) - x.get_sum2(b) << endl;
	}
	if (x.get_sum1(a) < x.get_sum2(b))
	{
		cout << " Subtraction = " << setprecision(3) << x.get_sum2(b) - x.get_sum1(a) << endl;
	}
}
